#!/mnt/home/student89/public_html/venv/bin/python3.12
import cgi
import cgitb
import json
import os
...
def weights_file(email, course_id):
    ...
    return os.path.join(WEIGHTS_DIR, safe + "_" + course_id + ".json")

def load_weights(email, course_id):
    try:
        with open(weights_file(email, course_id)) as f:
            return json.load(f)
    except:
        return {"categories": [], "assignments": {}, "manual": []}

def save_weights(email, course_id, data):
    with open(weights_file(email, course_id), "w") as f:
        json.dump(data, f)
...
def get_unsubmitted(conn, course_id):
    try:
        assignments = conn.account.get_assignments(course_id) or []
        result = []
        for a in assignments:
            status   = getattr(a, "submissions_status", "") or ""
            due_date = getattr(a, "due_date", None)
            name     = getattr(a, "name", "Unnamed")
            grade    = getattr(a, "grade", None)
            # Include anything with no submission regardless of due date
...
if action == "login":
    ...
    try:
        ...
        for c in all_courses:
            grades[c["id"]] = get_grades_summary(conn, c["id"])
    ...
    try:
        ...
        unsubmitted   = get_unsubmitted(conn, course_id)
        print(json.dumps({
            "ok":           True,
